...
mod test {
    ...
    fn bench_r_generate_sorted_state() {
        let start = Instant::now();
        let goal: Puzzle = Puzzle::new(r_generate_sorted_state(3), 3, 0);
        let elapsed = start.elapsed();
    ...
    fn bench_collections() {
        ...
        // for i in 0..5058 {
        //     let mut start_state: Vec<u8> = r_generate_sorted_state(3);
        ...
        let start_state: Vec<u8> = r_generate_sorted_state(3);
