...
use resolver::generate::r_generate_random_state;
use resolver::generate::r_generate_sorted_state;
...
fn run(puzzle: Puzzle) {
    ...
    // let mut goal_state: Vec<u8> = r_generate_sorted_state(size);
    ...
    // match puzzle.r_is_solvable(&goal) {
    //     true => {
